#ifndef GPRUSAK_SHORT_H_
#define GPRUSAK_SHORT_H_
...
template<typename T> INL int sign(T a){ return (a>0)-(a<0); }
...
template<typename T> INL void unique(vec<T> &v) {
  std::sort(v.begin(),v.end());
  v.erase(std::unique(v.begin(),v.end()),v.end());
}
template<typename T> INL inline vec<T> reverse(const vec<T> &A){ return vec<T>(A.rbegin(),A.rend()); }

}  // namespace gprusak

#endif  // GPRUSAK_SHORT_H_
