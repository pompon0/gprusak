#ifndef GPRUSAK_LOG_H_
#define GPRUSAK_LOG_H_

#include "gprusak/string.h"
#include "gprusak/short.h"
...
struct Profile {
  ...
  str show() {
    vec<str> lines;
    for(auto &[name,s] : scopes) lines.push_back(fmt("% : count = %, cycles = %, time = %\n",name,s.count,s.cycles,s.time));
    return join("",lines);
  }
};
...
extern Profile profile;
  ...
  #define FRAME(args...) gprusak::Frame _(VERBOSE,args);
#else
  #define DEBUG if(0)
